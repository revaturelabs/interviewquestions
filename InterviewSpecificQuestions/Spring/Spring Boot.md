...
---
2. Why do we need `Service layer`? Can’t we call `Repository layer` directly inside `Controller layer`?
...
- There is no restriction from compiler, and we can call Repository layer from Controller layer directly.
...
- Since property file(s) are readable by anyone who has access to codebase it is highly recommended not to keep the DB and other critical credentials inside property file(s).
...
---
4. As a developer you need to check your production application status daily, how Spring Boot could help here?
...
- One of the routine tasks for a developer is to check the status of already launched production application on periodic basis.
- To simplify this task, Spring Boot provids sub-project named Spring Boot Actuator.
...
- The information is accessed usually via HTTP endpoints, few of which are listed below:
...
- We commonly set active profile using `spring.profiles.active` property or command line argument.
...
- `Hibernate Validator` is most famous/used implementation of the Bean Validation API specification.
- There are 3 types of variables that can be validated using Bean Validation API:
  ...
  - Query parameters.
- Spring Boot comes with the validation starter, which need to be included in `pom.xml` file and intern added `hibernate-validator` dependency as below:
...
- It basically intercepts exceptions thrown by methods annotated with `@RequestMapping` and similar.
...
- We need to have a method annotated with @ExceptionHandler which takes Exception Class (any exception which you want to handle) as argument, if any of this exception is raised in the controller, then this handler method will handle it.
- If we have two handler method in same controller say for example one handler for Exception and another handler for RuntimeException, then the handler method which is closer to Exception Class hierarchy is triggered. 
...
---
14. Have you used HAL browser in Spring? How to use it?
...
---
15. What is the use of `ResponseEntity` class in Spring?
