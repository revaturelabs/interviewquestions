...
When the exception occurs in a method, the process of creating the exception object and handing it over to the runtime environment is called "throwing an  exception".
...
`throw` keyword is used to generate exceptions explicitly in our code, for example in a user authentication program, we should throw exceptions to the clients if the password is null. The throw keyword is used to throw exceptions to the runtime to handle it.
...
An object representing the exception is created and thrown in the method that caused the error. That method may choose to handle the exception itself, or pass it on. Either way, at some point, the exception is caught and processed. Exceptions can be generated by the Java run-time system, or they can be manually generated by your code.
